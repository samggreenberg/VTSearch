# Extract `vtcore` Library Plan

Goal: split VTSearch into two distributions in one repo:

- **`vtcore`** — reusable Python library: dataset origins, MediaSources, clippers/croppers, embedders, MLP/detector training and scoring, evaluation. No Flask, no Angular, no auto-writing JSON configs.
- **`vtsearch`** — the Flask + Angular application that wraps `vtcore`. Owns user-facing concerns: HTTP routes, auth, persistent user preferences, the SPA.
...
- Library import name: `vtcore` (recommended). Alternatives considered and rejected: `VTSearchLib` (non-idiomatic camelCase), `vtsearch.lib` (inverts the dependency direction — the app depends on the library, not vice versa), `vtsearch.core` namespace package (works but adds packaging complexity for marginal benefit).
- App import name: stays `vtsearch`.
- PyPI distribution names if/when published: `vtcore` and `vtsearch`.

## Phase 0 — Preparation (non-blocking, do anytime)

- [ ] Inventory the actual public surface that external consumers would call. Capture as a docstring-only API sketch in `docs/vtcore-api.md`. This is the contract the refactor must preserve.
...
**Exit criteria**: `grep -rn "flask" vtcore-candidate-paths/` returns zero hits.
...
1. Define a `vtcore.config.CoreConfig` dataclass with the knobs library code actually consumes (`safe_thresholds`, `calibrate_count`, `calibration_fraction`, `enrich_descriptions`, `data_dir`, `max_concurrent_dataset_downloads`, `max_concurrent_dataset_embeddings`, etc.).
...
**Exit criteria**: `grep -n "^medias\|^good_votes" vtcore-candidate-paths/` returns zero hits — those names exist only in the app shim.
...
**Exit criteria**: `grep -rn '"data/' vtcore-candidate-paths/` and `grep -rn "Path('data')" vtcore-candidate-paths/` are clean (only constants in `CoreConfig`).
...
- [ ] Library exposes `vtcore.utils.registry.PluginRegistry` (already generic).
...
- [ ] Add a custom `Unpickler.find_class` that maps `vtsearch.datasets.origin.Origin` → `vtcore.datasets.origin.Origin`, etc.
...
- [ ] Library tests must pass with `pip install vtcore` only — no Flask installed in their virtualenv. CI enforces.
...
1. Create `vtcore/` directory at repo root.
...
3. Search-and-replace `vtsearch.datasets` → `vtcore.datasets` etc., across the codebase.
4. Add `vtcore/pyproject.toml` and `pyproject.toml` workspace config so both distributions build.
5. App imports become `from vtcore.X import ...`.
6. Run full suite. Fix straggler imports.

## Phase 9 — Release plumbing

- [ ] Independent semver for `vtcore` (start at 0.1.0).
- [ ] CHANGELOG.md per distribution.
- [ ] `vtcore` README with quickstart: load a dataset, train a detector, score a folder.
...
├── shim/                  # NEW: glue between Flask g and vtcore contexts;
...
- **`eval/visualisation.py`** likely pulls in matplotlib — keep as an optional extra (`vtcore[viz]`) to avoid forcing the dep on lean consumers.
- **Heavy ML deps** (torch/transformers/CLAP/CLIP) — declare as required for now; revisit extras (`vtcore[embedders]`) if a consumer asks for a leaner install.
