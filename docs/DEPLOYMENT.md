...
Dependencies are declared in `requirements.txt` files. Top-level files
live under `requirements/`; each plugin (media type, importer, exporter)
has its own `requirements.txt` in its directory. The
`scripts/install-plugin-deps.sh` script auto-discovers all plugin
requirements and collects them into `requirements/plugins.txt`, which is
included by the top-level `requirements/base.txt`:

```
requirements/base.txt              ← Core deps (Flask, NumPy, etc.) + includes plugins.txt
requirements/plugins.txt           ← Auto-generated by scripts/install-plugin-deps.sh
requirements/gpu.txt               ← GPU PyTorch wheel + plugins
requirements/labbench.txt          ← LabBench (SigLIP-only) image deps
requirements/image-embedders.txt   ← All-image-embedders image deps (CPU)
requirements/image-embedders-gpu.txt ← All-image-embedders image deps (GPU)
vtsearch/media/*/requirements.txt      ← Per-media-type deps
vtsearch/converters/*/requirements.txt ← Per-converter deps
```
