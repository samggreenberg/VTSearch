...
Dependencies are declared in `requirements.txt` files. Core deps are in
the top-level `requirements.txt`; each plugin (media type, importer,
exporter) has its own `requirements.txt` in its directory. The
`install-plugin-deps.sh` script auto-discovers all plugin requirements
and collects them into `requirements-plugins.txt`, which is included by
the top-level `requirements.txt`:

```
requirements.txt              ← Core deps (Flask, NumPy, etc.) + includes requirements-plugins.txt
requirements-plugins.txt      ← Auto-generated by install-plugin-deps.sh (per-plugin deps)
