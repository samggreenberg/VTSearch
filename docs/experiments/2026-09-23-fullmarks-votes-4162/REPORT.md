# Does stamp-finding learn from votes? Vote curves on FullMarks (#4162)
...
FullMarks v5.0, the benchmark built for exactly this: pages with stamps and logos,
all of them boxed.
...
- **Corpus:** FullMarks **v5.0**, 36 roster classes. One class,
...
- for each positive page, the page's own SIFT features that fall inside its
  FullMarks box (`filter_features_to_box`). This is the template the app builds
...
VLAD ranks FullMarks at chance (AP 0.02), so almost no positive reaches the top
...
`/expscratch/sgreenberg/fullmarks/votes-4162/`, and the measurements here are
copied from there.
