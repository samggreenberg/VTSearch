# FullMarks — the first roster evaluation (#3904)
...
`scripts/experiments/fullmarks/eval_retrieval.py` reads the existing cells and
...
`fullmarks_config.CONTAMINATES` says a Tobacco800 class must not be scored against
...
`fullmarks_config.eligible_distractor`. Tested in
`tests_lib/datasets/test_fullmarks_eval.py`.
