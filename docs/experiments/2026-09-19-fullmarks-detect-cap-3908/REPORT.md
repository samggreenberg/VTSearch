# FullMarks — pinning the detection cap on retrieval, not on a synthetic warp (#3908)
...
1. **Keep 2 MP. It is the optimum, and by a wide margin.** At the 8,192-keypoint
   budget #3911 recommends, the shipped cap ranks the FullMarks roster at **mean AP
...
FullMarks v3.1 tier `s` (5,000 pages, 23 roster classes, 721 instances), the
...
150 pages sampled across the megapixel distribution (FullMarks tier `s` runs 0.13–63 MP,
...
- **FullMarks only.** #3908 asked for OpenLogo as well, and photographs may well put the
