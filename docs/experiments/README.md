...
|-------|----------|---------|
| [`2026-08-31-enrich-descriptions-3127/`](2026-08-31-enrich-descriptions-3127/PLAN.md) | Should `enrich_descriptions` still default to `False`, now that #3077 made `clap_general` the audio default (#3127)? Enrichment on/off for the default embedder of **every** media type — 22 eval datasets, 560 paired (dataset, category) queries — plus the `clap` control that re-measures the sign flip the issue rests on. | **Pre-registered, not yet run** — the plan is linked because no report exists yet. The setting is one global boolean serving four defaults, so the design is sized to see a per-domain disagreement rather than average over it. |
