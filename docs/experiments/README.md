...
| [`calibration-fold-combine/`](calibration-fold-combine/REPORT.md) | Pooling fold scores or averaging per-fold cuts (#3115) — two docstrings in the repo assert contradictory premises about whether fold scores are comparable; run with #3116's honest reference and `sd(threshold)`. | Pre-registered; run pending. |
