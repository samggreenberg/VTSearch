...
| `GaussianMixture(random_state=42)` and the `gmm_fit_array` subsample | EM initialisation, and which scores are kept above `_GMM_MAX_SAMPLES` | Rarely. Two components in one dimension over ≥ 500 scores is a well-posed fit; the subsample is a large uniform draw from a distribution whose quantiles are the only thing read off it. |
