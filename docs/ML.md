...
**What `safe_thresholds` on now computes: the fold-anchored mixture, not the blend.** The blend-vs-fusion question was measured by the #2852 deep-regime study and fusion won decisively, so the schedule was retired as the *combiner* (`fold_anchored_gmm_threshold`, adopting the study's recommendation). Per calibration fold *k*:
...
The anchor mass is **κ = 1** — each vote counts as *one* haystack point among the ~50k the mixture is fitted on. Performance degraded monotonically as κ grew in the sweep: honest anchors matter, anchor *weight* does not. Labels and haystack now feed one estimator rather than being averaged as rivals on a label-count schedule; the labels' authority grows with the data instead of on a hand-tuned ramp.

Measured at 100–300 votes on Visual Genome region voting, this beats pure cross-calibration by **−0.085** paired regret (winning 71–74 % of ~36k paired steps), beats the 6→20 ramp everywhere they differ, holds *both* error rates below x-cal's until 200 votes, and is **2.8×** steadier step to step. See [`docs/experiments/population-anchored-calibration/REPORT.md`](experiments/population-anchored-calibration/REPORT.md), which also records the two open threads: the study's blend control predates `slow_cap50`, and κ = 1 sits at the sweep's edge (a κ ∈ {0.1, 0.3, 1} × folds ∈ {2, 4} boundary sweep is still owed).

Degeneracy policy, per fold and then globally: a fold whose anchored fit degenerates (inverted means, collapsed component) falls back to that fold's **unanchored** GMM fit; if no fold yields a fit at all, the threshold falls back to the **blend** (`calculate_safe_threshold`), which is what still runs for datasets too small to form calibration folds — never to 0.5. The blend schedules therefore remain in the tree as that fallback and as harness arms, but they no longer decide the shipped cut.
