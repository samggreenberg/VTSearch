...
**The pinned seeds: which are real draws, and which are decorative.** Several `42`s appear in this pipeline, and they are not equivalent — only one of them is a coin flip whose outcome a user can feel:

| Site | What the seed decides | Does it move the detector? |
|---|---|---|
| `CALIBRATION_SPLIT_SEED` (`vtscore/training/thresholds/conformal.py`) | **which labels land on the Calibrate side** of each fold | **Yes.** Different splits leave different held-out scores, so the conformal quantile cuts somewhere else. This is the pipeline's one genuine draw. |
| `fit_linear_svm_head(seed=…)` → `LinearSVC(random_state=…)` | liblinear's coordinate order | No. The shipped head is a convex solve with a unique optimum; the seed permutes the path to it, not the answer. |
| the `gmm_fit_array` subsample | which scores are kept above `_GMM_MAX_SAMPLES` | Rarely. A large uniform draw from a distribution whose quantiles are the only thing read off it. (The mixture fit itself no longer has a seed: since #3585 it is a deterministic 2-means init and our own EM, not `GaussianMixture(random_state=42)`. Nothing about *that* answer was random, but the fit was still not as stable as this row implied - re-initialising the incumbent moved 6.2% of a text sort's verdicts, which is why the swap had to be measured rather than argued.) |
| `_split_dither_rng` | which side the odd vote goes to at a fractional split | Deliberately **not** seeded from 42 — it is seeded from a digest of the labelset, so the tie-break is a pure function of the votes rather than of a global constant (issue #3286). |

All of them are *pinned* rather than drawn fresh, and that is the point: issue #2934 fixed exactly the opposite state of affairs, where the uncached threshold paths drew their fold splits from the unseeded global `np.random` and a detector's verdicts moved between two runs on identical votes. Reproducibility is the feature.

What the pin costs is that every user gets the *same* arbitrary draw and no study reports the spread around it. That is a measurement gap, not a reason to unpin: the eval harness's `calibration_seed` knob (issue #3794) holds the data fixed and sweeps the split instead, so the spread can be quoted as an error bar on single-seed numbers. See [`docs/EVAL.md`](EVAL.md#the-calibration-draw-calibration_seed-default-the-apps-own-pin).
...
- **The shipped combiner looks right at exactly the number a spot check reads** (4.3 % flagged against a nominal 5 %) while sitting 0.103 from uniform — not the worst of the five, and not the best either: `median` is a third closer overall (0.071) and over-flags at α instead. No candidate is good at both, which is why this went unnoticed.
