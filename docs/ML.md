...
The blend-vs-fusion question has since been measured: the #2852 deep-regime study found a **fold-anchored mixture** (a semi-supervised GMM fitted per calibration fold on the fold model's haystack scores with that fold's held-out labeled scores clamped as anchors, rank-transferred back to the final scale) beats pure cross-calibration by −0.085 paired regret at 100–300 votes and beats the 6→20 ramp everywhere they differ, while holding both error rates below x-cal's until 200 votes. Production still ships the blend schedules; adoption is gated on the follow-up in [`docs/experiments/population-anchored-calibration/REPORT.md`](experiments/population-anchored-calibration/REPORT.md) (the study's blend control predates `slow_cap50`, and the winning anchor mass sits at the sweep edge).
