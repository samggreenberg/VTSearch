...
The blend-vs-fusion question has since been measured twice. The #2852 deep-regime study found a **fold-anchored mixture** (a semi-supervised GMM fitted per calibration fold on the fold model's haystack scores with that fold's held-out labeled scores clamped as anchors, rank-transferred back to the final scale) beats pure cross-calibration by −0.085 paired regret at 100–300 votes on Visual Genome. The #2861 follow-up swept the anchor mass two decades lower across six environments and **changed the answer twice**: the optimum is interior at `κ=0.3` with the *midpoint* cut, not `κ=1` with the rate crossing (better in 6/6 environments); and measured against the schedules that actually ship (`slow_cap50`/`cap50`, which postdate the first study) fusion is a clear win on **region voting** (−0.026) but a dead heat on COCO binary voting and a small loss on an 838-image set — the gain tracks how many *positive* anchors the regime supplies. Production still ships the blend schedules; see [`docs/experiments/population-anchored-calibration/REPORT.md`](experiments/population-anchored-calibration/REPORT.md).
