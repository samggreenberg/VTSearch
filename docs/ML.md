...
The `calibrate_count` setting defaults to `2` (`DEFAULT_CALIBRATE_COUNT` in `vtscore/config.py`) and can be raised or lowered (`VTSEARCH_CALIBRATE_COUNT`) to trade calibration quality (and Inclusion-knob resolution — more folds means more pooled calibration scores) for latency. (The eval runner uses its own default of `2` for a separate, non-interactive path — see [`docs/EVAL.md`](EVAL.md).) When `safe_thresholds` is enabled and the mix-in schedule puts zero weight on the calibrated value (with both shipped schedules, at 6 labels or fewer), the cross-calibration step is skipped entirely, since paying for the fold trainings would be pure waste. Which schedule applies depends on the voting mode — a patch dataset blends under `slow` (handing over at 40 labels) and a single-vector dataset under `cap50` (which never hands over completely); see `docs/experiments/mixin-schedule/REPORT.md`. The skip is derived from the schedule rather than hard-coded, so a different mix-in curve (see `vtscore/training/blend_schedules.py`) automatically stops skipping where it starts trusting the calibrated cut. With `safe_thresholds` **off**, the cross-calibrated value is the cutoff the detector actually uses, so it is computed at every label count — every training entry point (vote-driven Train, labelset re-derivation, Find, and detector-load-from-origins) cross-calibrates below 6 labels rather than falling back to 0.5.

`safe_thresholds` is **on by default** (issue #2799). Measured as an on/off A/B on the production linear head over Visual Genome region voting, the blend cut inclusion-weighted cost 0.546 → 0.472 and FNR 0.416 → 0.346 across the 6–20-vote window where it has authority (81 % of 226 paired cells improved, p = 5.6e-23), *without* raising FPR — the cut lands closer to the model's own oracle rather than simply lower. The gain persists past the blend's authority (21+ votes: cost −0.020) because the threshold steers Autopilot's next Hard pick, so a better cut also buys better votes. It is still a per-user setting, and a user who turns it off keeps it off. See [`docs/experiments/safe-thresholds/REPORT.md`](experiments/safe-thresholds/REPORT.md).

The blend-vs-fusion question has since been measured twice. The #2852 deep-regime study found a **fold-anchored mixture** (a semi-supervised GMM fitted per calibration fold on the fold model's haystack scores with that fold's held-out labeled scores clamped as anchors, rank-transferred back to the final scale) beats pure cross-calibration by −0.085 paired regret at 100–300 votes on Visual Genome. The run-B follow-up (PR #2864) swept the anchor mass two decades lower across six environments and **changed the answer twice**: the optimum is interior at `κ=0.3` with the *midpoint* cut, not `κ=1` with the rate crossing (better in 6/6 environments); and measured against the schedules that actually ship (`slow_cap50`/`cap50`, which postdate the first study) fusion is a clear win on **region voting** (−0.026) but a dead heat on COCO binary voting and a small loss on an 838-image set — the gain tracks how many *positive* anchors the regime supplies. PR #2861 shipped the run-A setting (`κ=1`, rate cut) as the production threshold before run B finished, so both of those are open regressions against shipped code, not choices about it; see [`docs/experiments/population-anchored-calibration/REPORT.md`](experiments/population-anchored-calibration/REPORT.md).
