...
| `settings-appearance` | (manifest only) | A new **HuggingFace** tab was added to the Settings tab strip (for "Sign in with HuggingFace"), so the tab row framed alongside the Appearance pane now shows an extra tab. | 2026-06-30 · gated-dataset-download-ux |
