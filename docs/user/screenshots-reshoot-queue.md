...
| `dashboard-loaded` | USER_GUIDE.md#what-vtsearch-does | The RAM / Disk usage bars now redden as they fill via the shared bar's `high-bad` polarity (green→yellow→red continuous gradient) instead of the old accent→warning(≥80%)→bad(≥95%) threshold fill (issue #2486). **Also (issue #2694):** the Detectors section now shows a `Drafts (N)` / `AutoRun (N)` tab strip, laid out inline in the section header between the "Detectors" title and the combine/delete/**+** actions (it no longer occupies a row of its own above the grid); the captured PNG predates the tabs. | 2026-07-14 |
| `dashboard-manage` | USER_GUIDE.md#dashboard--managing-datasets-and-detectors | Same RAM / Disk usage-bar recolor as `dashboard-loaded`: the bars in the framed action-bar row now use the `high-bad` green→yellow→red gradient instead of the old threshold fill (issue #2486). **Also (issue #2694):** the Detectors section gained the `Drafts (N)` / `AutoRun (N)` tab strip — laid out inline in the section header, to the right of the "Detectors" title, not as its own row above the grid — and the framed detector-row ⋯ menu now includes a "Move to AutoRun" item (with Delete last); the captured PNG shows neither. | 2026-07-14 |
...
| `browse-view` | USER_GUIDE.md#browse--exploring-a-dataset-spatially | The top-bar `Data:` / `Detector:` context pulldowns are now display-only on the browse view: the caret (▾) is gone and the triggers are disabled (the pair can only be switched from the Dashboard). The captured full-page PNG still shows the old interactive pulldowns with carets. **Also (corner symbology):** the bin corner convention flipped — multi-item ("pile") bins are now drawn rounded (a disc in hex mode, a rounded square in square mode) and single-item bins are drawn sharp (full hexagon / square), the inverse of the captured PNG where singletons were rounded and piles sharp. Applies to the canvas map and the minimap. **Also (image-tile crop/pad):** image bins on the main canvas now use a balanced "half crop, half pad" fit (geometric-mean scale) instead of the old cover crop, so each non-hovered image tile shows a background-coloured gap inside the bin border rather than filling it edge-to-edge. The captured PNG (a synthetic image dataset) shows the old edge-to-edge cover crop. **Also (singleton border):** single-item thumbnail bins now carry a hard-edged (sharp-cornered) border in the colormap's dedicated one-item colour — the "1" end of the bin-size scale — instead of the old faint dark hairline, so a lone item stands out against the background. The captured PNG shows the old hairline. | 2026-07-17 |
