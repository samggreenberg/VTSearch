...
| `three-panel` | USER_GUIDE.md#the-three-panel-layout | Panels now snap tight to their grid columns on load (not just on divider drag), so the left/right panel widths — and the centre boundary between them — shift from the old restored-width layout. | 2026-07-09 |
