...
| `settings-appearance` | USER_GUIDE → Settings → Appearance | (1) A new **HuggingFace** tab was added to the Settings tab strip, so the tab row framed alongside the Appearance pane shows an extra tab. (2) A new always-on "Browser motion: Allowed / Blocked" status line now sits directly under the Show Animations toggle, reporting whether the browser/OS prefers-reduced-motion gate is suppressing animations. | 2026-06-30 · gated-dataset-download-ux + vtsearch-animations-missing |
