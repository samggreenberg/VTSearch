...
| `dashboard-loaded` | USER_GUIDE.md#what-vtsearch-does | The RAM / Disk usage bars now redden as they fill via the shared bar's `high-bad` polarity (green→yellow→red continuous gradient) instead of the old accent→warning(≥80%)→bad(≥95%) threshold fill (issue #2486). **Also (issue #2694):** the Detectors section now shows a `Drafts (N)` / `AutoRun (N)` tab strip between the section header and the grid; the captured PNG predates the tabs. | 2026-07-14 |
| `dashboard-manage` | USER_GUIDE.md#dashboard--managing-datasets-and-detectors | Same RAM / Disk usage-bar recolor as `dashboard-loaded`: the bars in the framed action-bar row now use the `high-bad` green→yellow→red gradient instead of the old threshold fill (issue #2486). **Also (issue #2694):** the Detectors section gained the `Drafts (N)` / `AutoRun (N)` tab strip, and the framed detector-row ⋯ menu now includes a "Move to AutoRun" item (with Delete last); the captured PNG shows neither. | 2026-07-14 |
