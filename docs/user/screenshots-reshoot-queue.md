...
| `browse-view` | USER_GUIDE → "Browse — exploring a dataset spatially" | The hex/square bin-shape toggle was removed from the canvas toolbar (shape is now fixed by media type), and the framed image dataset now renders **square** tiles instead of hexagons. Also: the Selection panel header's text **Clear** button was replaced by a tri-state select-all checkbox ([ ] none / [-] some / [x] all-in-view). Also: the top-left **Back to Dashboard** button was removed (it duplicated the top-bar **Dashboard** button) and the remaining toolbar button was renamed from **Re-project** to **Rebuild map** (plain-language sweep). | 2026-06-29 · square-hex-thumbnails; 2026-07-01 · selection-tristate-checkbox; 2026-07-01 · remove-redundant-back-button; 2026-07-01 · simplify-user-text |
...
| `find-stats` | USER_GUIDE → "Find — scoring and verifying" | Plain-language sweep of the Detector Stats modal: the **Precision** row label became **Kept rate**, the chart section title changed from "False pos / false neg vs. inclusion" to "Missed vs. wrong matches by inclusion", and the two legend labels changed from "False positives" / "False negatives" to "Wrong matches" / "Missed matches". | 2026-07-01 · simplify-user-text |
| `importer-picker` | USER_GUIDE → "Loading a dataset" | Plain-language sweep: the demo-catalogue per-row readiness badge for downloaded-but-unembedded datasets was relabeled from **Needs Embed** to **Needs setup**. (Only drifts if a framed row is in that state.) | 2026-07-01 · simplify-user-text |
