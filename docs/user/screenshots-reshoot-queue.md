...
| `browse-view` | USER_GUIDE → "Browse — exploring a dataset spatially" | The hex/square bin-shape toggle was removed from the canvas toolbar (shape is now fixed by media type), and the framed image dataset now renders **square** tiles instead of hexagons. Also: the Selection panel header's text **Clear** button was replaced by a tri-state select-all checkbox ([ ] none / [-] some / [x] all-in-view). Also: the top-left **Back to Dashboard** button was removed from a full-dataset browse (it duplicated the top-bar **Dashboard** button), leaving only **Re-project** in that toolbar. | 2026-06-29 · square-hex-thumbnails; 2026-07-01 · selection-tristate-checkbox; 2026-07-01 · remove-redundant-back-button |
