...
| [plans/](plans/) | Open design plans for future work (multi-media import, patch embedders, clipper chain, etc.); one file per plan |
...
Models are loaded lazily on first use. The full embedder roster, with
model IDs and dimensions, is the generated table in
[ML.md § Embedding Models](ML.md#embedding-models);
`scripts/download_models.sh` prefetches the most common ones (see
[DEPLOYMENT.md](DEPLOYMENT.md#network-dependencies)).

### Converters

Media converters transform content between types (e.g. document pages to
images, video to audio). See `vtscore/converters/` and the generated
converter roster in [EXTENDING-media.md § Built-in
...
Auto-discovered plugin families share a common `PluginRegistry`
architecture. The authoritative family inventory is the generated table
in [EXTENDING-plugins.md](EXTENDING-plugins.md#shared-plugin-architecture).
