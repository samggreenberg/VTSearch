...
- **`label_scores`** (new, optional cache) — the labelset fold-scores from calibration. Inclusion-independent once the model architecture is fixed, so caching them lets an Inclusion change re-run only `find_optimal_threshold` (no fold-model refits).
