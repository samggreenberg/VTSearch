...
`name_evidence.py`, `coco_folds.py` (retired with Visual Genome — see `docs/plans/coco-better.md`), `vg_name_families.py`,
...
- **DONE — #4008.** `pilebuild/loaders/coco_better.py`, 200 lines against `vg_scale`'s 1,064, importing `band_for` unchanged. Against the real corpus: 75 cells, 7,500 positives, 0 short of `SCALE_N_POS`. `iscrowd` dropped explicitly, as the census did.
...
- **DONE — #4038 made this the settled position.** `vg_scale*` cells stay on disk and load by path; nothing can rebuild one. Studies conditioned on them keep working, and nothing new is built on them.
  ...
  **Keep the old cells readable under their own name.** `pile_config` warns that
  ...
  re-run — but it is a reason not to reuse the key. `coco_better` is a new
...
- **DONE — #4034.** `coco_better_export.py` selects positives and negatives from the embedded corpus at any prevalence the data supports, so `SCALE_N_POS`, `SCALE_N_NEG` and π are filters rather than rebuilds. All 75 cells reproduce the designated membership exactly at `SCALE_N_POS`.
...
- [x] #4056 — ***C* is 53** (PR pending). The count rule admitted 54; `wine glass` is
  ...
  **An earlier reading of +0.24 AP was wrong in sign and is corrected in the
  ...
  25-class build is preserved at `keep/coco-better-25-20260920/`. Per-class pools
