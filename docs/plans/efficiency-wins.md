...
- **Per-field settings validation** — every autogenerated settings getter runs
  `model.model_validate({key: value}).model_dump()` over the **entire**
  Pydantic model (`vtsearch/settings.py:554-568`): ~40 fields constructed and
  serialized to return one, firing `default_factory` for every absent field.
  For `ServerSettings` reads that includes `_default_concurrent_embeddings` →
  `resolve_device()` (`vtscore/config.py:127`), which calls
  `torch.cuda.is_available()` on every read (the kernel smoke test itself is
  memoized in `_cuda_runnable`, but the availability probe + cpu/memory
  probes re-run). Settings getters are called all over request handling, and
  `_umap_params()` (`vtsearch/routes/projection.py:122`) reads two
  ServerSettings keys on the projection status path.
  **Fix:** two independent halves — (a) `functools.lru_cache(maxsize=None)` on
  `resolve_device()` (device identity is fixed for the process; note
  `tests_lib` stubs embedders so no test depends on it changing mid-process —
  verify with a grep for `resolve_device` in tests and for monkeypatching of
  `DEVICE`); (b) replace `_validate_field`'s whole-model round-trip with a
  cached per-field validator: build
  `TypeAdapter(model.model_fields[key].annotation)` once per `(model, key)`
  in a module-level dict — **but check first** that the fields' constraints
  live in the annotation (`Annotated[..., BeforeValidator(...)]` style, see
  `vtsearch/settings_models.py:146-183`); if a field's validation lives in a
  `@field_validator` method instead, the TypeAdapter won't run it — for those
  fields keep the whole-model path (a `_WHOLE_MODEL_KEYS` allowlist) or move
  the validator into the annotation. Preserve the exact error-message shape
  (`"Invalid {key}: {msg}"`) and the corrupt-value fallback to the field
  default (`getter`'s `except ValueError` branch, settings.py:577-580).
  **Gotchas:** `model_validate({key: value})` today also applies
  cross-field/model validators if any exist — grep `settings_models.py` for
  `model_validator`; if present, single-key validation was already only
  seeing one key, so per-field is behaviorally identical, but confirm.
  Settings tests are extensive: `./run-tests.sh core api`.
  **Files touched:** `vtsearch/settings.py`, `vtscore/config.py`, possibly
  `vtsearch/settings_models.py`.
  Impact: medium (pervasive per-request overhead, worst on GPU hosts).
