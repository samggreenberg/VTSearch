...
### 13.1 Eliminate first-vote retrain stall ★★★ M
...
71 legacy functions still carry `# noqa: C901` markers (down from 77 — see `git grep "# noqa: C901"`). Each one is a candidate for incremental refactoring; the markers can be deleted as functions are simplified under complexity 10. *Burned down so far:* `multi_find` and `find_check_labels` (the two worst offenders, CC 62 and 22, broken into named helpers in `vtsearch/routes/detectors/find.py`); `load_dataset_from_folder` / `load_dataset_from_folder_chunked` / `load_dataset_from_pickle` / `load_dataset_from_pickle_chunked` (CC 50/51/37/29 — collapsed onto a shared per-file helper layer in `vtsearch/datasets/loader_folder.py` and `loader_pickle.py`). *Worst offenders remaining:* `import_local_folder` (36), `run_converters_on_folder` (34), `demo_dataset_list` (31), `_run_pipeline` (31), `export_labels` (29), `DatasetImporter.effective_source_specs` (27), `load_pipeline_file` (26), `_resolve_or_train_detector` (25). No deadline — refactor opportunistically when touching the code.
