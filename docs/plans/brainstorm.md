...
72 legacy functions carry `# noqa: C901` markers (see `git grep "# noqa: C901"`). Each one is a candidate for incremental refactoring; the markers can be deleted as functions are simplified under complexity 10. Worst offenders: `multi_find` (41), `load_dataset_from_folder_chunked` (35), `load_dataset_from_folder` (34), `_run_origin_load_in_background` (31), `task` (27, inside load_pipeline), `load_dataset_from_pickle` (26), `import_local_folder` (24), `_make_per_side_setting` (24). No deadline — refactor opportunistically when touching the code.
