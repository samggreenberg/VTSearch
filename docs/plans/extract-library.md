...
- [x] **Independent semver for `vtscore` (start at 0.1.0).** Version lives in `vtscore/__init__.py` as a manually-bumped string constant — explicitly *not* git-derived, in contrast to `vtsearch.__version__` (which is the HEAD commit timestamp). The module docstring documents the bump-on-release contract.
- [x] **CHANGELOG.md per distribution.** `vtscore/CHANGELOG.md` documents the 0.1.0 release scope (every library subpackage carved out of the monolith plus the architecture invariants — no Flask imports, no settings imports, no hardcoded `data/` paths, no persisted vectors / MLPs). The top-level `CHANGELOG.md` covers `vtsearch` and notes that the library extraction is internally significant but user-invisible at the app surface; it also flags the entry-point group rename (`vtsearch.<family>` → `vtscore.<family>` for library-tier families, settings families unchanged) for third-party plugin authors.
- [x] **`vtscore` README with quickstart.** `vtscore/README.md` walks an external consumer through the three core flows — load a dataset folder, train an MLP detector from labels, score a fresh folder against the trained detector — using only library-tier imports (`vtscore.media`, `vtscore.datasets.loader`, `vtscore.training`). Links out to `docs/vtscore-api.md` for the full surface inventory and `docs/EXTENDING.md` for plugin authoring.
- [x] **PyPI publication cadence — deferred.** No standalone `vtscore` distribution on PyPI yet; both packages ship from the single root `pyproject.toml` (Phase 8 open follow-up). The README and CHANGELOG both call this out and link the actual PyPI publication to a future moment when a real external consumer asks for it. Splitting `pyproject.toml` is the gating work for that follow-up.
...
All numbered items below have shipped. The plan itself is complete; the only durable open thread is the PyPI publication / `pyproject.toml` split tracked under "Open follow-ups (Phase 8)". Picking up from here, ordered smallest-first:
...
12. ~~**Phase 9 release plumbing.**~~ **Shipped (PyPI publication deliberately deferred).** `vtscore/__init__.py` now documents the manual-bump semver contract (0.1.0 starting point, *not* git-derived — contrast `vtsearch.__version__`). `vtscore/README.md` walks an external consumer through load-folder → train-MLP → score-folder using only library-tier imports. `vtscore/CHANGELOG.md` captures the 0.1.0 release scope and the architecture invariants; the top-level `CHANGELOG.md` covers `vtsearch` and flags the entry-point group rename for third-party plugin authors. PyPI publication cadence stays deferred until a real external consumer asks for it (which is also gated on the Phase 8 follow-up that splits the single root `pyproject.toml` into one per distribution).
...
- Both packages currently ship from the single root `pyproject.toml` declared as the `vtsearch` distribution (with `setuptools.packages.find` configured to include both `vtsearch*` and `vtscore*`). When/if `vtscore` ever ships as its own PyPI package, that needs a `vtscore/pyproject.toml` of its own and the root config has to slim down to just the app's deps. Defer until a real external consumer asks for it — Phase 9 deliberately leaves this open (see Phase 9 shipped notes).
- ~~`vtscore/__init__.py` carries a hardcoded `__version__ = "0.1.0"` placeholder.~~ **Resolved by Phase 9.** The constant is now the canonical source of truth for `vtscore` version, intentionally hand-managed (independent semver, in contrast to `vtsearch.__version__`'s git-derived timestamp). The module docstring documents the bump-on-release contract.
