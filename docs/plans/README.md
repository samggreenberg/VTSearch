...
| Plan | Status | Summary |
|------|--------|---------|
| [codebase-reorg.md](codebase-reorg.md) | **Complete** | Every actionable item has shipped; the remaining items (`dashboard.component.ts` split, plugin-discovery unification, `cli.py` split, `auth/` collapse) are explicitly **Skip** with rationale. Doc retained as a record of the decisions. |
