"""Fit Toponymy on an image dataset and dump the signpost tree.

Mirrors the architecture in ``docs/plans/vtsbrowse-toponymy.md``:

- ``embedding_vectors`` = the SigLIP matrix (what VTSearch holds in memory),
- ``clusterable_vectors`` = a dedicated 5-D UMAP of that matrix
  (``metric="cosine"``, per Toponymy's examples),
- ``text_embedding_model`` = an adapter over the SigLIP text branch
  (``MediaEmbedder.embed_text``) so keyphrases live in the same space,
- ``objects`` = the per-image texts from ``make_texts.py`` (the
...
Subset scenario (Find→Browse): ``--categories a,b,c`` and/or ``--per-cat N``
restrict the fit to a slice, recomputing both UMAPs on the slice — the same
thing VTSearch does when browsing the results of a Find.

Outputs ``RESULTS/<dataset>/topo_<variant>_<namer><suffix>.json`` with
per-layer topic names, per-image cluster labels, the cluster tree, 2-D
anchors, timings, and LLM call stats.

Usage::

    python run_toponymy.py caltech101 tags_oi600 keyphrase
    python run_toponymy.py stanford_dogs caption_qwen3b hf
    python run_toponymy.py mixed caption_blip hf --corpus-desc "'a mixed collection...'"
"""
...
class SiglipTextEncoder:
    ...
    def encode(self, texts, show_progress_bar=False, **kwargs):
        ...
        for t in texts:
            ...
            if v is None:
                v = np.zeros(self._dim or 768, dtype=np.float32)
            self._dim = len(v)
...
def main() -> None:
    ...
    ap.add_argument("--categories", default=None, help="comma-separated GT categories to keep (subset scenario)")
    ...
    if args.categories or args.per_cat or args.n_cats:
        keep_cats = set(args.categories.split(",")) if args.categories else None
        if args.n_cats:
            pool = sorted(keep_cats) if keep_cats else sorted({m["category"] for m in meta})
            keep_cats = set(pool[: args.n_cats])
        counts: dict = {}
        idx = []
        for i, m in enumerate(meta):
            c = m["category"]
            if keep_cats is not None and c not in keep_cats:
                continue
            counts[c] = counts.get(c, 0) + 1
            if args.per_cat and counts[c] > args.per_cat:
                continue
            idx.append(i)
        ...
        n = len(idx)
        subset_note = {"categories": sorted(counts), "per_cat": args.per_cat, "n": n}
    print(f"{args.dataset}/{args.variant}/{args.namer}: {n} images, emb dim {emb.shape[1]}")
