#!/usr/bin/env python
"""Embed a built FullMarks corpus into pile-format cells, on the GRID.
...
One cell = one ``fullmarks_<tier>__<embedder>.pkl`` of media dicts carrying
...
the full ``dataset x embedder`` cross-product, so adding FullMarks *and*
``sift_vlad`` there would silently schedule ``sift_vlad`` cells for all six
existing datasets and three deep-embedder cells for each FullMarks tier — a
...
def _cells_io() -> Any:
    ...
    return _load_by_path("_fullmarks_cells_io", _CALIB_DIR / "_cells_io.py")


def _pile_config() -> Any:
    return _load_by_path("_fullmarks_pile_config", _PILE_DIR / "pile_config.py")


def cell_name(tier: str, embedder: str) -> str:
    return f"fullmarks_{tier}__{embedder}.pkl"
    ...
    *start_index* is the id the first media takes, so :func:`build_cell` can
    ...
    for offset, page in enumerate(sorted(pages, key=lambda p: p.page_id)):
        ...
        medias[index] = {
            ...
            "categories": ordered,
            "regions": regions,
            "origin": {"importer": "fullmarks", "params": {"embedder": embedder, "page_id": page.page_id}},
...
def build_cell(
    ...
    **Streamed, in chunks of *chunk* pages** (``0`` disables it and writes the
    ...
    if out.exists() and not force:
        print(f"skip fullmarks_{tier} x {embedder} (exists: {out.name})")
    ...
    print(f"=== fullmarks_{tier} x {embedder}: {len(pages)} page(s) in {n_chunks} chunk(s) of {size}")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    # import unresolvable, and fullmarks_s x sift_vlad died on
