"""FullMarks review as Good/Bad questions in VTSearch.
...
``bank`` only ever reads detectors whose name starts with ``fullmarks``, and
...
start ``fullmarks <class> -- <question>`` so the dashboard groups by class.
"""
...
PREFIX = "fullmarks"
ROOT = Path("/expscratch/sgreenberg/fullmarks/binary")
...
COMPLETENESS2 = Path("/expscratch/sgreenberg/fullmarks/completeness2/verdicts.suggested.jsonl")
...
def fullmarks_detectors(detectors: Sequence[dict[str, Any]]) -> list[dict[str, Any]]:
...
def collect_votes(base: Optional[str], root: Path, cleared: Path) -> tuple[dict[str, dict[str, str]], dict[str, int]]:
    ...
    for path in sorted(root.glob("*/votes_*.json")):
        ...
        if name and got:
            ...
            seen["archive"] += 1
    if base:
        for d in fullmarks_detectors(api(base, "/api/detectors/registry").get("detectors", [])):
...
def bank(
    ...
    for d in fullmarks_detectors(api(base, "/api/detectors/registry").get("detectors", [])) if base else []:
        name = d["name"]
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    e.add_argument("--cross", type=Path, default=Path("/expscratch/sgreenberg/fullmarks/ucsf-3921/cross.json"))
