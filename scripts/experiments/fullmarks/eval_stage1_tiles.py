"""Tiled VLAD as Stage 1 for FullMarks structural search (#3928, first probe).
...
#: tile.  A FullMarks mark is ~0.1-0.3 of a page's width; the two layouts bracket it.
_ALL_LAYOUTS = {"t4": (0.25, 0.18), "t3": (0.34, 0.25)}
#: ``FULLMARKS_TILE_LAYOUTS`` (space-separated names) restricts the run -- tile
#: vectors for a 50,000-page tier are ~46 GB per layout.
LAYOUTS = {
    k: v
    for k, v in _ALL_LAYOUTS.items()
    if k in os.environ.get("FULLMARKS_TILE_LAYOUTS", " ".join(_ALL_LAYOUTS)).split()
}
MIN_TILE_KP = 20
KS = (100, 500, 1000)
