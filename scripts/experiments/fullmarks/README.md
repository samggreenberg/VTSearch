# FullMarks — eval data for stamp detection
...
that at 200k pages; `CONTAMINATES` in `fullmarks_config.py` fixes it by
...
into `/expscratch/sgreenberg/fullmarks/binary/<queue>/`, `load` makes each queue a
dataset plus same-named detector (`fullmarks <class> -- <question>`, one per
class), and `bank` writes the votes back as `audit/<task>/verdicts.from_vtsearch.jsonl`
for `audit_to_corrections.py`. `bank` reads only `fullmarks` detectors and deletes
...
```bash
bash launch_fullmarks.sh siglip                                   # GPU, ~7 min
...
sitting rather than two, and `launch_fullmarks.sh slate` renders both into one
...
`embed_corpus.py` writes `fullmarks_<tier>__<embedder>.pkl` into the shared
...
```bash
python make_report.py --corpus <dir> --out docs/experiments/<date>-fullmarks/report.html
```
