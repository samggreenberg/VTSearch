...
**Put `VTS_FULLMARKS_RAW` and `VTS_FULLMARKS_OUT` on `/expscratch`, never on the
...
```bash
export VTS_FULLMARKS_RAW=/expscratch/$USER/fullmarks/raw
export VTS_FULLMARKS_OUT=/expscratch/$USER/fullmarks/corpus
...
codec that RAR4 needs. `launch_fullmarks.sh` prepends `~/.local/bin` to `PATH`
...
instead of hammering through. `VTS_FULLMARKS_FETCH_WORKERS=1` restores the
...
```bash
python shortlist.py --corpus $VTS_FULLMARKS_OUT --write-roster --name fullmarks-v1
```
...
```bash
python build_corpus.py --sources spods,staver,tobacco800,ucsf --roster $VTS_FULLMARKS_OUT/roster.json --new-version
```
...
```bash
bash launch_fullmarks.sh slate           # ~minutes, CPU, no refetch
# when it finishes, the log names the bundle:
scp $GRID:$VTS_FULLMARKS_OUT/audit/fullmarks-audit-<date>.tar.gz .
```
...
```bash
bash launch_fullmarks.sh siglip          # ~minutes, GPU, embeds ~1.3k crops once
scp $GRID:$VTS_FULLMARKS_OUT/audit/fullmarks-audit-siglip-<date>.tar.gz .
```
...
crop from full-resolution scans that live under `$VTS_FULLMARKS_OUT`, and pulling
...
```bash
python audit_to_corrections.py --task merge      --corpus $VTS_FULLMARKS_OUT   # dry run
python audit_to_corrections.py --task merge      --corpus $VTS_FULLMARKS_OUT --apply
python audit_to_corrections.py --task membership --corpus $VTS_FULLMARKS_OUT --apply
```
...
Cells land in `$VTS_PILE/embeddings/fullmarks_<tier>__<embedder>.pkl`. Verify
before trusting:
...
```bash
python build_corpus.py ... --pin-tiers $VTS_FULLMARKS_OUT/build_report.json
```
...
`--new-version`, and `launch_fullmarks.sh build` refuses before submitting unless
`VTS_FULLMARKS_PIN_TIERS` or `VTS_FULLMARKS_NEW_VERSION=1` is set. The choice is
