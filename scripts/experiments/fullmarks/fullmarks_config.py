"""Configuration for the FullMarks corpus — stamps and logos in scanned documents.

FullMarks is one corpus assembled from several sources, in three strata:
...
RAW = Path(
    os.environ.get("VTS_FULLMARKS_RAW", "/expscratch/{u}/fullmarks/raw".format(u=os.environ.get("USER", "user")))
)
...
OUT = Path(
    os.environ.get("VTS_FULLMARKS_OUT", "/expscratch/{u}/fullmarks/corpus".format(u=os.environ.get("USER", "user")))
)
...
TIERS: dict[str, int] = {
    "s": int(os.environ.get("VTS_FULLMARKS_TIER_S", "5000")),
    "m": int(os.environ.get("VTS_FULLMARKS_TIER_M", "50000")),
    "l": int(os.environ.get("VTS_FULLMARKS_TIER_L", "200000")),
}
...
TIER_SALT = os.environ.get("VTS_FULLMARKS_TIER_SALT", "fullmarks-v1")
...
MIN_INSTANCES = int(os.environ.get("VTS_FULLMARKS_MIN_INSTANCES", "10"))
...
def min_instances_for(source: str) -> int:
    ...
    if os.environ.get("VTS_FULLMARKS_MIN_INSTANCES"):
...
MIN_MARK_PX = int(os.environ.get("VTS_FULLMARKS_MIN_MARK_PX", "32"))
...
MIN_MARK_AREA_FRAC = float(os.environ.get("VTS_FULLMARKS_MIN_MARK_AREA_FRAC", "0.0002"))
...
MAX_MARK_AREA_FRAC = float(os.environ.get("VTS_FULLMARKS_MAX_MARK_AREA_FRAC", "0.25"))
...
CONTAMINATES: dict[str, frozenset[str]] = {
    ...
    # docs/experiments/2026-09-22-fullmarks-ucsf-contamination/.
...
LETTERHEAD_BAND_FRAC = float(os.environ.get("VTS_FULLMARKS_LETTERHEAD_BAND_FRAC", "0.22"))
...
CLUSTER_BACKEND = os.environ.get("VTS_FULLMARKS_CLUSTER_BACKEND", "phash")
...
CLUSTER_THRESHOLD = float(os.environ.get("VTS_FULLMARKS_CLUSTER_THRESHOLD", "0.10"))
...
def cluster_threshold_for(source: str) -> float:
    ...
    if os.environ.get("VTS_FULLMARKS_CLUSTER_THRESHOLD"):
...
QUERY_CORE_SPREAD = float(os.environ.get("VTS_FULLMARKS_QUERY_CORE_SPREAD", "3.0"))
...
QUERY_CROP_MIN_REACH_FRAC = float(os.environ.get("VTS_FULLMARKS_QUERY_CROP_MIN_REACH_FRAC", "0.5"))
...
SYNTH_INSTANCES_PER_CLASS = int(os.environ.get("VTS_FULLMARKS_SYNTH_PER_CLASS", "30"))
...
SYNTH_SEED = int(os.environ.get("VTS_FULLMARKS_SYNTH_SEED", "20260825"))
...
MERGE_SLATE_INSTANCES = int(os.environ.get("VTS_FULLMARKS_MERGE_INSTANCES", "3"))
...
MERGE_SLATE_NEAR_PAIRS = int(os.environ.get("VTS_FULLMARKS_MERGE_NEAR_PAIRS", "120"))
...
AUDIT_EMBEDDER = os.environ.get("VTS_FULLMARKS_AUDIT_EMBEDDER", "siglip2_l")
...
AUDIT_MAX_PER_CLASS = int(os.environ.get("VTS_FULLMARKS_AUDIT_MAX_PER_CLASS", "24"))
...
AUDIT_MIXED_MAX_WITHIN = float(os.environ.get("VTS_FULLMARKS_AUDIT_MIXED_MAX_WITHIN", str(max(AUDIT_SPLIT_SWEEP))))
...
SLATE_DESCRIPTOR = os.environ.get("VTS_FULLMARKS_SLATE_DESCRIPTOR", "phash")
