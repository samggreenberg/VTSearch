#!/usr/bin/env python
"""Render the contact sheets for the FullMarks human passes.
    ...
    * **A closed world that only covers what was looked at.**  ``REVIEWED-ALL``
    ...
    for start in range(0, len(cells), per_sheet):
        ...
        _paste_grid(
            ...
            f"FullMarks merge slate — classes [{start}]..[{start + len(cells[start : start + per_sheet]) - 1}] "
    ...
    for start in range(0, len(pairs), cfg.MERGE_PAIRS_PER_SHEET):
        ...
        _paste_grid(
            pair_cells,
            f"FullMarks — the {len(pairs)} closest pairs, {start}..{start + len(chunk) - 1} — "
...
# FullMarks merge slate — {n} classes, {p} near pairs on the appendix sheets.
#
