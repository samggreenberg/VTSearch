...
def _cpu_umap(X, nn, md, seed):
    ...
    reducer = umap.UMAP(
        n_components=2, n_neighbors=min(nn, X.shape[0] - 1), min_dist=md,
...
def verify():
    ...
    w = csv.writer(out)
    w.writerow(["dataset", "embedder", "setting", "n_neighbors", "min_dist", "compact", "ratio_cpu_mean", "ratio_cpu_std"])
    ...
    for tag_path in sorted(C.MATRIX_DIR.glob("*.npz")):
        ...
        kw = {"lineage": ({leaf[i]: list(map(str, lineage_rows[i])) for i in range(len(leaf))} if lineage_rows is not None else {})}
        if ml is not None:
        ...
        for name, s in settings.items():
            ...
            ratios[name] = rs
            w.writerow([dataset, embedder, name, s["n_neighbors"], s["min_dist"], s["compact"],
                        round(float(np.mean(rs)), 5), round(float(np.std(rs)), 5)])
            out.flush()
        ...
        print(f"  {tag:26s} winner={np.mean(ratios['winner']):.3f} baseline={np.mean(ratios['baseline']):.3f} "
    ...
    for emb, rows in by_emb.items():
        ...
        print(f"  {emb:9s} winner−baseline Δ={dw:+.4f} on CPU · transfers on {int(frac*len(rows))}/{len(rows)} datasets")
    json.dump(
        {"per_dataset": [{"dataset": d, "embedder": e, "winner": float(w), "baseline": float(b), "transfers": bool(t)}
                         for d, e, w, b, t in verdicts]},
        open(C.RESULTS_ROOT / "cpu_verify.json", "w"), indent=2,
    )
