...
# VTS_REPO is load-bearing: common.setup_env() puts it at the *front* of
# sys.path, so without it every task would import vtscore from the default
# worktree (vts-calib) instead of this branch.
SHARED="export VTS_REPO=$WT CALIB_DATASETS=$CALIB_DATASETS CALIB_VG_EMBEDDERS=$CALIB_VG_EMBEDDERS \
