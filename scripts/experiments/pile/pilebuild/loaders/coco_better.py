"""``coco_better``: `vg_scale`'s question, asked of COCO 2017 with no Visual Genome.
...
``/expscratch/sgreenberg/keep/coco-better-25-20260920/`` and is reproducible from
...
def read_coco_labels(
    ...
    *classes*: an image with no entry is "no answer", while an image with an
    ...
    for split in SPLITS:
        ...
        if not path.exists():
            raise SystemExit(f"coco_better: missing {path}; run coco_anchor.py --fetch")
    ...
    log(f"  coco_better: {len(labels):,} images, {crowd:,} iscrowd regions dropped")
...
def _lvis_boxes(lvis_dir: Path, wanted: dict[str, str]) -> dict[int, dict[str, list[list[float]]]]:
    ...
    for split in pc.LVIS_SPLITS:
        ...
        if not path.exists():
            raise SystemExit(f"coco_better: missing {path}; LVIS v1 is the pile referee (#3985)")
        with path.open() as fh:
            data = json.load(fh)
...
def _zip_members() -> dict[str, tuple[Path, str]]:
    ...
    for zip_path in (pc.COCO_VAL_ZIP, pc.COCO_TRAIN_ZIP):
        if not zip_path.exists():
            raise SystemExit(f"coco_better: missing {zip_path}")
...
def _plan(dataset: str) -> _Plan:
    ...
    if full:
        ...
        # export-time queries, which is the point of COCO Better.
    ...
    log(
        f"  coco_better: {len(cells)} cells, {sum(len(v) for v in chosen.values()):,} positives, "
        f"{len(negatives):,} negatives (+{len(spares):,} spares)"
    )
    if short:
        log(f"  coco_better: WARNING {len(short)} cells under SCALE_N_POS: {', '.join(short[:6])}")
...
def load(dataset: str, medias: dict[int, dict], embedder_name: str) -> None:
    ...
    missing = 0
    log(f"  coco_better: emitting {len(emit_ids):,} medias{plan.where}")
    ...
    if missing:
        log(f"  coco_better: WARNING {missing} designated images absent from the archives")
...
def relabel(dataset: str, medias: dict[int, dict]) -> dict:
    ...
    log(f"  coco_better: relabelled {len(medias):,} medias{plan.where}; fields changed: {dict(changed)}")
    return {"n_medias": len(medias), "changed": dict(changed), "plan_ids_not_in_cell": unemitted}


def check(dataset: str) -> str:
    """What a ``coco_better`` rebuild reads, named as the loader names them.
