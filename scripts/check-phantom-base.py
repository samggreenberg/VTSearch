...
def _report_reverted_window(base_ref: str) -> bool:
    ...
    print(
        ...
        f"\n"
        f"Every path those commits touched is at its pre-merge content here, "
        f"and\n{len(reverted)} of them are byte-identical to the version "
        f"above. That is a stale checkout\ncommitted onto a fresh HEAD, and "
        f"it does not have to delete anything to do it:\nthe damage can sit "
        f"entirely in hunks inside files that survive, which is what\nthe "
        f"deletion check above is blind to.\n"
        f"\n"
    ...
    print(
        ...
        f"\n"
        f"If you meant to revert those commits, re-run with:\n"
        f"\n"
