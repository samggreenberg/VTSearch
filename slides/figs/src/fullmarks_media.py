"""Real pages and real marks from FullMarks' four sources, for the deck's cards.

The FullMarks corpus itself lives on the GRID, so a checkout cannot open it —
...
readers (`scripts/experiments/fullmarks/sources/`) and its own mask settings,
...
Downloads go under the gitignored `data/fullmarks-sources/`, idempotently:
...
ROOT = _REPO_ROOT / "data" / "fullmarks-sources"
_FULLMARKS = _REPO_ROOT / "scripts" / "experiments" / "fullmarks"
...
def _modules() -> tuple[Any, Any, Any, Any, Any]:
    if str(_FULLMARKS) not in sys.path:
        sys.path.insert(0, str(_FULLMARKS))
    import fullmarks_config
    from sources import _common, spods, staver, tobacco800

    return fullmarks_config, _common, spods, staver, tobacco800
...
def page(source: str, stem: str) -> tuple[Any, list[tuple[str, tuple[int, int, int, int]]]]:
    ...
    raise SystemExit(f"fullmarks media: no reader for source {source!r}")
...
def datasheet_roster() -> list[str]:
    ...
    text = (_FULLMARKS / "DATASHEET.md").read_text()
