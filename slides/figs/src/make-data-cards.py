...
def _crop(ann_id: int, pad: float = 0.06) -> Any:
    from PIL import Image
    ...
    with Image.open(images / meta["file_name"]) as frame:
        ...
        return frame.crop(
            (
                max(0, int(x - pad * w)),
                max(0, int(y - pad * h)),
                min(frame.width, int(x + w * (1 + pad))),
                min(frame.height, int(y + h * (1 + pad))),
            )
        )
...
def frame_problem_merge() -> Any:
    ...
    for row, (pair, merged) in enumerate(COCO_MERGE_PAIRS):
        top = 0.955 - row * 0.285
        x = 0.395
        for ann_id, label in pair:
            crop = _crop(ann_id)
            aspect = crop.width / crop.height * dc.FIG_H / dc.FIG_W
            width = min(height * aspect, 0.14)
            tall = width / aspect
            _picture(fig, [x, top - height + (height - tall) / 2, width, tall], crop)
            fig.text(
                x + width / 2,
                top - height + (height - tall) / 2 - 0.008,
                f"“{label}”",
                ha="center",
                va="top",
            ...
            x += width + 0.025
        fig.text(0.735, top - height / 2, "→", va="center", fontsize=dc.FLOOR_PT + 8, color=dc.CUT)
        fig.text(
            0.775,
            top - height / 2,
        ...
        if x - 0.025 > 0.72:
            raise SystemExit(f"coco merge row {row}: crops run to {x:.3f}, into the arrow")
    return fig
...
QUARRY_ZOOM = 404484
QUARRY_ZOOM_ABSENT = ("bird", "bus", "book", "chair", "bench", "clock", "bicycle")
