...
`--check` also requires a build's notes to **name every page of the group** —
one `**c** —` per reveal, though one note may cover several. A frame nobody
wrote a line for is invisible until you are standing in front of a room.
...
copies to keep in step, and one fragment used five ways cannot drift.
