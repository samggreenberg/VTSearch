"""`coco_better`'s three decisions, on a corpus small enough to reason about.
...
def mod():
    ...
    from pilebuild.loaders import coco_better
...
def test_scale_media_carries_the_importer_and_skips_undecodable_bytes():
    ...
    ok = scale_media(data=_jpeg_bytes(), importer="coco_better", **common)
    assert ok is not None
    assert ok["origin"]["importer"] == "coco_better"
    assert ok["coco_scored"] is True and ok["category"] == ""
    assert scale_media(data=b"not a jpeg", importer="coco_better", **common) is None


class TestFullCorpusMode:
    """`coco_better_full` embeds the corpus; `coco_better` embeds a draw from it.
    ...
    def test_the_dataset_declares_the_flag(self):
        ...
        assert pc.DATASETS["coco_better_full"]["full_corpus"] is True
        assert pc.DATASETS["coco_better_full"]["kind"] == "coco_better", "one loader, two datasets"
        assert not pc.DATASETS["coco_better"].get("full_corpus")
        assert pc.DATASETS["coco_better_full"].get("on_request"), "6.8x the designated build"
...
class TestPatchShards:
    ...
    def test_every_shard_is_declared(self):
        ...
        shards = [d for d in pc.DATASETS if d.startswith("coco_better_full_s")]
        assert len(shards) == pc.COCO_BETTER_SHARDS
        for i, name in enumerate(sorted(shards, key=lambda d: int(d.rsplit("s", 1)[1]))):
            spec = pc.DATASETS[name]
            assert spec["shard"] == (i, pc.COCO_BETTER_SHARDS)
            assert spec["full_corpus"] is True, "a shard is a slice of the full corpus"
            assert spec["kind"] == "coco_better", "one loader, many datasets"
    ...
    def test_a_shard_carries_every_class(self, mod, tmp_path: Path):
        ...
        anchor = _corpus(tmp_path)
        labels, _, _ = mod.read_coco_labels(anchor, ("bus",))
        per_shard = [{i for i in labels if i % pc.COCO_BETTER_SHARDS == k} for k in range(pc.COCO_BETTER_SHARDS)]
...
class TestLargestInstance:
    ...
    def test_coco_better_uses_it(self):
        import pile_config as pc
...
class TestRelabel:
    ...
    def full(self, mod, tmp_path: Path, monkeypatch):
        ...
        monkeypatch.setitem(pc.DATASETS, "fixture_full", {"kind": "coco_better", "full_corpus": True})
        mod._CORPUS.clear()
        yield "fixture_full"
        mod._CORPUS.clear()
