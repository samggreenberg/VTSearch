...
300-per-class parquet mirror whose ``image``/``label`` columns decode into a
``<class>/<idx>.png`` folder-per-class tree (the integer ``label`` indexes
``RVL_CDIP_CATEGORIES``), reusing the Caltech/Food-101 collect path.
"""
...
class TestDownloadRvlCdip:
    def test_labels_map_to_class_folders(self, tmp_path):
        ...
        # label 2 -> "email", 6 -> "invoice"; 99 is out of range -> skipped.
        rows = [
        ...
        assert next((result / RVL_CDIP_CATEGORIES[6]).glob("*.png"), None) is not None  # invoice
    ...
    def test_cached_skips_download(self, tmp_path):
        ...
        imgs = tmp_path / "rvl_cdip" / "images" / "letter"
        imgs.mkdir(parents=True)
        (imgs / "0.png").write_bytes(_png_bytes())
...
class TestRvlCategoriesList:
    def test_has_16_unique_in_mirror_order(self):
        ...
        # Order is the parquet mirror's ClassLabel ordering (index == label int).
        assert RVL_CDIP_CATEGORIES[2] == "email"
        assert RVL_CDIP_CATEGORIES[6] == "invoice"
