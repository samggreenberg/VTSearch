...
if TYPE_CHECKING:
    # The accessors below are generated at import time by the
    # ``for _model in (ServerSettings, UserSettings)`` loop further down
    # (via :func:`_make_scalar_accessors`), plus the six per-side pairs
    # built by :func:`_make_per_side_setting`. Because they are installed
    # into ``globals()`` rather than written out, static type-checkers
    # cannot see them; the stubs here declare the signatures so
    # ``vtsearch.settings.get_<key>()`` / ``set_<key>()`` resolves from
    # other modules (and inside this one).
    #
    # This block must mirror every field of both models, minus the three
    # ``_SKIP_AUTOGEN`` keys (``autofind_detectors``, ``saved_datasets_dir``,
    # ``detectors_dir``), which have hand-written ``def``s below and so need
    # no stub. ``tests/api/test_settings_dispatch.py`` enforces that in both
    # directions - a new model field without a stub, or a stub naming a field
    # that no longer exists, fails the suite. Keep them in sync by editing
    # this block, never by adding a ``# type: ignore[name-defined]``.
    ...
    # are their only consumers. They are stubbed anyway so the block stays a
    # complete mirror of the models (enforced by
    # ``tests/api/test_settings_dispatch.py``).
    def get_bin_details_docked() -> dict[str, bool]: ...
    def set_bin_details_docked(value: dict[str, bool]) -> None: ...
    def get_browse_details_panel_width() -> int: ...
    def set_browse_details_panel_width(value: int) -> None: ...
    def get_browse_details_metadata_width() -> int: ...
    def set_browse_details_metadata_width(value: int) -> None: ...
    def get_grid_icon_size_popup() -> dict[str, str]: ...
    def set_grid_icon_size_popup(value: dict[str, str]) -> None: ...
    def get_popup_metadata_shown() -> dict[str, bool]: ...
    def set_popup_metadata_shown(value: dict[str, bool]) -> None: ...
    def get_popup_preview_size() -> dict[str, int]: ...
    def set_popup_preview_size(value: dict[str, int]) -> None: ...
...
def apply_user_solo_embedder_per_media_type(value: dict[str, str] | None) -> None:
    ...
    """
    if value is None:
        set_solo_embedder_per_media_type({})
        return
    ...
    for mt, emb in value.items():
        ...
        if isinstance(emb, str) and emb.strip():
            ...
            cleaned[mt.strip()] = ""
    set_solo_embedder_per_media_type(cleaned)
...
def _server_default_settings_source() -> dict[str, Any] | None:
    ...
    """
    return get_default_settings_source()
