"""Shared trainer registry for the learned-sort comparison sweeps.

Both eval sweeps — the label-count curve (:mod:`vtscore.eval.label_curve`) and
the Autopilot voting simulation (:mod:`vtscore.eval.voting_iterations`) — need
the same set of candidate rankers plus the trainer-agnostic cross-calibration
threshold.  They used to live in ``label_curve``; this module extracts them so
the voting simulation can dispatch through the identical registry without
importing the whole label-curve sweep.

A **trainer** is a callable ``(X_train, y_train, seed) -> predict_fn`` where the
returned ``predict_fn`` maps an ``(N, D)`` embedding matrix to per-row
``P(positive)`` scores in ``[0, 1]`` (ensembles additionally return a per-item
std; see :data:`PredictFn`).  :data:`TRAINERS` holds the fixed-name entries;
:func:`resolve_trainer` additionally parses **parameterised** SVM names such as
``"svm_rbf@C=3,gamma=scale"`` so the kernel/hyperparameter screen can sweep the
SVM configuration space without a registry entry per point.
"""
...
# Registry of fixed-name trainers.  Adding a new *named* candidate model here is
# the only place it needs to be plugged in for both sweeps.  Parameterised SVM
# configurations do not need an entry - see :func:`resolve_trainer`.
...
def _parse_trainer_spec(name: str) -> tuple[str, dict[str, Any]]:
    ...
    if not base.startswith("svm_"):
        raise KeyError(
            f"Unknown trainer {name!r}; choices: {sorted(TRAINERS)} or svm_<kernel>[@C=..,gamma=..,degree=..]"
        )
