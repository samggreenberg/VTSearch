...
class TestListMediaIds:
    ...
    def test_stub_fields_only(self, client):
        ...
        resp = client.get("/api/medias/ids")
        data = resp.get_json()
        allowed = {"id", "media_type", "embedder", "embedders"}
        for media in data:
            assert "id" in media
...
class TestMediaThumbnail:
    ...
    def test_on_the_fly_thumbnail_is_memoised(self, client):
        # A media lacking precomputed ``thumbnail_bytes`` is generated on the
        # fly on first request, and the generated bytes are cached back onto the
        # in-memory media dict so subsequent cold fetches stream them verbatim
        # (no repeat decode+resize). Datasets backed by an external media dir
        # arrive without ``thumbnail_bytes``, so this keeps fresh browse-canvas
        # zooms/pans off the slow regeneration path.
        from vtsearch.state import get_media  # noqa: PLC0415

        media = get_media(1)
        assert media is not None
        original = media.get("thumbnail_bytes")
        media["thumbnail_bytes"] = None
        try:
            first = client.get("/api/medias/1/thumbnail")
            assert first.status_code == 200
            memoised = media.get("thumbnail_bytes")
            assert isinstance(memoised, bytes) and len(memoised) > 0
            assert memoised == first.data
            # Second request now takes the precomputed fast path: same bytes,
            # and its ETag is a conditional 304-able fingerprint of the thumb.
            second = client.get("/api/medias/1/thumbnail")
