...
class TestSettingsModule:
    ...
    def test_add_favorite_processor(self, isolated_settings):
        settings_mod.add_favorite_processor("my det", "server_detector_file", {"filepath": "/tmp/det.json"})
    ...
    def test_add_overwrites_same_name(self):
        settings_mod.add_favorite_processor("a", "server_detector_file", {"filepath": "1.json"})
        settings_mod.add_favorite_processor("a", "server_detector_file", {"filepath": "2.json"})
        procs = settings_mod.get_favorite_processors()
        assert len(procs) == 1
        assert procs[0]["field_values"]["filepath"] == "2.json"

    def test_remove_favorite_processor(self):
        settings_mod.add_favorite_processor("x", "server_detector_file", {"filepath": "x.json"})
    ...
    def test_persistence_survives_reset(self, isolated_settings):
        ...
        settings_mod.add_favorite_processor("p", "server_detector_file", {"filepath": "p.json"})
...
class TestEnsureFavoriteProcessorsImported:
    def test_imports_detector_file_processor(self, tmp_path):
        ...
        settings_mod.add_favorite_processor("settings_test_det", "server_detector_file", {"filepath": str(det_path)})
    ...
    def test_skips_already_imported(self, tmp_path):
        ...
        settings_mod.add_favorite_processor("existing", "server_detector_file", {"filepath": "/nonexistent.json"})
    ...
    def test_handles_bad_importer_gracefully(self):
        ...
        settings_mod.add_favorite_processor("bad_proc", "totally_fake_importer", {"filepath": "x.json"})
