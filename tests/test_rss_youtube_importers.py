...
class TestRssFeedImporterMetadata:
    """RSS feed importer metadata (moved to old_io)."""

    def _get_importer(self):
        from old_io.datasets.importers.rss_feed import IMPORTER

        return IMPORTER

    def test_rss_feed_display_name(self):
        imp = self._get_importer()
        assert imp.display_name == "Generate from RSS Podcast Feed"

    def test_rss_feed_icon(self):
        imp = self._get_importer()
        assert imp.icon == "\U0001f3b5"

    def test_rss_feed_has_url_field(self):
        imp = self._get_importer()
        keys = [f.key for f in imp.fields]
        assert "url" in keys

    def test_rss_feed_has_media_type_field(self):
        imp = self._get_importer()
        keys = [f.key for f in imp.fields]
        assert "media_type" in keys

    def test_rss_feed_has_max_episodes_field(self):
        imp = self._get_importer()
        keys = [f.key for f in imp.fields]
...
class TestRssFeedImporterCLI:
    ...
    def test_adds_expected_args(self):
        from old_io.datasets.importers.rss_feed import RSSDatasetImporter
...
class TestRssFeedImporterRun:
    ...
    def test_empty_feed_raises_value_error(self):
        ...
        with mock.patch("old_io.datasets.importers.rss_feed.feedparser", create=True) as mock_fp:
...
class TestYouTubePlaylistImporterMetadata:
    """YouTube playlist importer metadata (moved to old_io)."""

    def _get_importer(self):
        from old_io.datasets.importers.youtube_playlist import IMPORTER

        return IMPORTER

    def test_youtube_display_name(self):
        imp = self._get_importer()
    ...
    def test_youtube_has_url_field(self):
        imp = self._get_importer()
        keys = [f.key for f in imp.fields]
        assert "url" in keys

    def test_youtube_has_media_type_field(self):
        imp = self._get_importer()
        keys = [f.key for f in imp.fields]
        assert "media_type" in keys

    def test_youtube_media_type_options(self):
        imp = self._get_importer()
    ...
    def test_youtube_has_max_videos_field(self):
        imp = self._get_importer()
        keys = [f.key for f in imp.fields]
...
class TestYouTubePlaylistImporterCLI:
    ...
    def test_adds_expected_args(self):
        from old_io.datasets.importers.youtube_playlist import YouTubePlaylistDatasetImporter
