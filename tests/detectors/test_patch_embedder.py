...
class TestLeafSeedingAndAssignment:
    """The `seeding` / `assignment` knobs on propose_leaves. Defaults
    (`topk` / `spatial`) must reproduce the original baseline byte-for-byte;
    the alternatives must measurably change the partition."""

    def test_defaults_identical_to_baseline(self):
        out = _make_output(h=6, w=6, d=16, seed=2)
        base = propose_leaves(out.patch_grid, out.patch_saliency, k=6)
        explicit = propose_leaves(out.patch_grid, out.patch_saliency, k=6, seeding="topk", assignment="spatial")
        for a, b in zip(base, explicit, strict=True):
            assert a.box == b.box
            np.testing.assert_array_equal(a.vec, b.vec)
            np.testing.assert_array_equal(a.cell_mask, b.cell_mask)
    ...
    def test_feature_assignment_changes_membership(self):
        # Pure-cosine assignment (alpha=1) binds cells by feature, not geometry,
        # so the partition must differ from the spatial Voronoi for some seed.
        differs = False
        for seed in range(5):
            out = _make_output(h=6, w=6, d=16, seed=seed)
