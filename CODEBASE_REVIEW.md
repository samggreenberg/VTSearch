...
### 6. `favoriteDetectors` vs `autorunDetectors` state mismatch (frontend)
**File:** `static/app.js:57-58, 433, 590`

The detector picker reads from `autorunDetectors` (only set in `renderDashboardModels` at line 1408), but after creating a detector via the processor importer flow, only `favoriteDetectors` is updated (line 590). The picker may show a stale or empty detector list.

**Fix:** Update `autorunDetectors` alongside `favoriteDetectors` at line 590, or consolidate into a single variable.

---

### 7. Missing error handling in `checkDatasetStatus` (frontend)
...
### 8. Race condition: `medias` dict read without lock
...
### 9. `_on_progress` callback mutated without thread safety
...
### 10. `bool("false")` is `True` in boolean settings
...
### 11. Trainable model slug collisions
...
### 12. Unbounded model cache in progress tracking
...
### 13. Zip extraction without path traversal protection in downloader
...
### 14. Image file handle leak in demo dataset loading
...
### 15. `set_inclusion` persists settings outside lock
...
### 16. Progress tracker silently resets extra fields
...
### 17. Combine-datasets importer mutates source dicts in-place
...
### 18. DatasetImporter singleton shared mutable state
...
### 19. Inclusion slider has no debounce (frontend)
...
### 20. Dead code: `loadFavoriteDetectors` function (frontend)
**File:** `static/app.js:672-678`

Fetches from `/api/favorite-detectors` but is never called. Uses a different endpoint than the code at line 590 (`/api/autorun-detectors`), suggesting it is outdated.

---

### 21. `aria-valuenow` never updated on progress bars
...
### 22. Video volume not applied on render
...
### 23. Overly permissive test assertions
...
### 24. Redundant `client` fixture shadows conftest
...
### 25. Bare `except Exception: pass` on cache load
...
### 26. Silent exception swallowing in detector threads
...
### 27. Audio clipper divide by zero
...
### 28. Partial download files prevent retry
...
### 29. Undocumented lock ordering across three lock hierarchies
...
### 30. `not c` instead of `c is None` for media lookup
