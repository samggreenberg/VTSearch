...
| §5.1-2 | Hardcoded `px`/`rem` in `padding`/`margin`/`gap`/`font-size` | Regex on the property list |
| §5.3   | Hex color literals in component SCSS | `#xxxxxx` outside `_variables.scss` |
| §5.6   | `font-weight: 700` / `bold` | Regex |
| §5.7   | Heading tags (`h1`–`h6`) restyled in component SCSS | Top-level `hN {` rule blocks |
| §5.10  | `transition: all <custom-duration>` | Regex, ignores tokenised durations |
...
| §5.13  | `font: inherit` inside component SCSS (specificity trap) | Regex |
| §5.14  | `flex-direction: column` blocks without an explicit `gap` | Brace-depth aware SCSS parser |
| §5.15  | Shared utility classes redeclared locally | Top-level `.{class} {` outside shared files |
...
- **§5.15 redeclared utility class** is almost always real drift -
...
- **§5.1-2 raw `px`/`rem`** is sometimes a deliberate off-scale value
...
2. **Highlight the clear bugs first** - the two token-resolution checks
   ...
   anything in §5.13 or §5.15 is usually genuine; §5.6 (`font-weight:
   700`) and §5.10 (`transition: all`) are almost always real.
3. **Group §5.14 hits by likely category** (real form/section
