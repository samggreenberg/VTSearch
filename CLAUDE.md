...
- `vtsearch/concurrency/` — Async job manager (`async_jobs.py` — `AsyncJob`, `JobManager`, `eval_jobs`, `learned_sort_jobs`), memory-aware worker capping (`memory_budget.py` — `cap_workers_by_memory`), and dataset/sort/eval progress trackers (`progress.py` — `update_progress`/`get_progress` + sort/eval variants, `cancel_dataset_progress`, `check_dataset_cancelled`). A second progress module still lives under `vtsearch/models/progress.py` (model-cache + loading-task tracker) and is queued for a follow-up merge with this one
