...
Never use the em-dash character `—` (U+2014) anywhere in this repository: not in code comments, not in docstrings, not in UI strings, not in documentation, not in commit messages. Em-dashes are an LLM writing tell. Use a regular hyphen `-` instead (with surrounding spaces when acting as a parenthetical dash, e.g. `text - more text`). This rule covers every file type: `.py`, `.ts`, `.html`, `.scss`, `.md`, `.json`, `.txt`, and all others.
...
- In-memory caches are fine and encouraged: `DetectorContext.label_embeddings`, `DetectorContext.model`, etc. These live for the lifetime of the process and are repopulated from origins on the next start.
...
The single exception is **dataset pickle files**, which are by design a snapshot of media and their embeddings. They ARE the dataset, not a cache.
...
- Linter errors from `ruff check` (including the flake8-bandit `S` ruleset), formatting drift from `ruff format --check`, typos from `codespell`, dependency issues from `deptry`, known CVEs from `pip-audit`, type errors from `pyright`, and OpenAPI snapshot drift. All of these run as the first steps of `./run-tests.sh`, so the test loop catches them before pytest. There is no CI backstop: VTSearch has no GitHub Actions workflows, and `./run-tests.sh` is the source of truth, so do not push a change without running it.
...
- `tests/`: **App-tier** test suite (uses Flask `client`, `vtsearch.routes`, `vtsearch.settings`, `vtsearch.auth`, etc.). Files are bucketed by group folder; the folder name **is** the pytest marker (`tests/core/test_*.py` → marker `core`). New test files inherit their group from where they live with no registry to update.
...
**ONLY look at this final summary block** (bordered by `====` lines) to determine pass/fail. Many test names contain the word "error" (e.g., `test_memory_errors.py`, `TestErrorResponseFormat`). These test **error-handling behavior** and are not failures.
