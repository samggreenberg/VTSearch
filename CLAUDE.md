...
- `vtsearch/embedding/` — Embedder façades and torch runtime. `helpers.py` (`embed_audio_file`/`embed_image_file`/`embed_video_file`/`embed_paragraph_file`/`embed_text_query` — thin wrappers around `MediaEmbedder`), `loader.py` (`initialize_models`, `preload_autoload_media_types`, `get_torch_device`, `get_clap_model`/`get_xclip_model`/`get_e5_model` getter wrappers), `matrix.py` (cached contiguous `(N, D)` embedding matrix on `DatasetContext`)
